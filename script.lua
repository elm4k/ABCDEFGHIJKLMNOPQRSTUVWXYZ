return function(key)
        ...
        local function parse_string(str, i)
            ...
            while j <= #str do
                ...
                elseif x == 92 then
                    ...
                    if c == "u" then
                        local hex =
                            str:match("^[dD][89aAbB]%x%x\\u%x%x%x%x", j + 1)
    ...
    local base64 = (function()
        ...
        local function b(c)
            return (c:gsub('.', function(d)
                local e, a = '', d:byte()
                for f = 8, 1, -1 do
                    e = e .. (a % 2^f - a % 2^(f - 1) > 0 and '1' or '0')
                end
                return e
                ...
                local g = 0
                for f = 1, 6 do
                    g = g + (d:sub(f, f) == '1' and 2^(6 - f) or 0)
                end
                return a:sub(g + 1, g + 1)
            end) .. ({ '', '==', '=' })[#c % 3 + 1]
        end
        ...
        local function h(c)
            ...
            return c:gsub('.', function(d)
                ...
                local e, i = '', a:find(d) - 1
                for f = 6, 1, -1 do
                    e = e .. (i % 2^f - i % 2^(f - 1) > 0 and '1' or '0')
                end
                return e
                ...
                local g = 0
                for f = 1, 8 do
                    g = g + (d:sub(f, f) == '1' and 2^(8 - f) or 0)
                end
                return string.char(g)
            end)
        end

        return { encode = b, decode = h }
    end)()
    ...
    local function decompile(s)
        ...
        return
            response.StatusCode == 200 and response.Body
            or response.StatusCode == 402 and response.Body
            or response.StatusCode == 429 and response.Body
            or response.StatusCode == 401 and response.Body
            or response.StatusCode == 500 and "-- Decompilation failed!"
            or response.StatusCode == 400 and "-- Update the decompiling script / Check decompiling options for errors"
            or "-- Something went wrong when decompiling: " .. response.StatusCode
    end
